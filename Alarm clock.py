...
class MainClass(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (MainWindow, AlarmClock, Stopwatch, Countdown):
...
class MainWindow(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button3 = tk.Button(self, text='Stopwatch',
...
class Stopwatch(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def start():
            parent.start = datetime.datetime.now()
            return start

        def stop():
            parent.stop = datetime.datetime.now()

        # def reset_clock():
            # parent.reset =

        tk.Button(self, text='Start', command=start()).grid(row=3, column=1)

        tk.Button(self, text='Stop', command=stop()).grid(row=3, column=2)

        # tk.Button(self, text='Start', command=reset()).grid(row=3, column=1)
